...
interface NotasProveedorModalProps {
  open: boolean;
  onClose: () => void;
...
const NotasProveedorModal: React.FC<NotasProveedorModalProps> = ({
  open,
  onClose,
  proveedor,
  onAgregarNota,
  ...
  const nombreProveedor = proveedor?.razonSocial || '';
  const notasOrdenadas = proveedor?.notas ? [...proveedor.notas].sort((a, b) =>
  ...
  return (
    <Dialog open={open} onClose={onClose} maxWidth="md" fullWidth>
      <DialogTitle>
        Notas e Incidentes - {nombreProveedor}
      </DialogTitle>
      <DialogContent>
        {canEditNotas && (
          <Box sx={{ mb: 3, mt: 1 }}>
        ...
        {notasOrdenadas.length === 0 ? (
          <Alert severity="info" sx={{ mt: 2 }}>
            No hay notas registradas para este proveedor
          </Alert>
        ) : (
...
export default NotasProveedorModal;
